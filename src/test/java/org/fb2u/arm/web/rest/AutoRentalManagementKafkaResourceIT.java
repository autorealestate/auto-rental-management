...
import org.fb2u.arm.config.KafkaProperties;
...
class AutoRentalManagementKafkaResourceIT {
    ...
    void setup() {
        ...
        AutoRentalManagementKafkaResource kafkaResource = new AutoRentalManagementKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/auto-rental-management-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/auto-rental-management-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
